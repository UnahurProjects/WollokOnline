...
describe("MockGitHubService", () => {
  it("genera repo desde template, commitea, archiva y lista", async () => {
    ...
    expect(repo.url).toContain("UnahurProyects/parcial2-ana");
    expect(repo.archived).toBe(false);

    // el template trae archivos
    const files = await gh.readWorkspace({ org: "UnahurProyects", repoName: "parcial2-ana" });
    ...
    const last = await gh.getLastCommit({ org: "UnahurProyects", repoName: "parcial2-ana" });
    expect(last?.sha).toBe(commit.sha);

    const list = await gh.listRepos({ org: "UnahurProyects", prefix: "parcial2-" });
    expect(list.map((r) => r.name)).toContain("parcial2-ana");

    await gh.archiveRepo({ org: "UnahurProyects", repoName: "parcial2-ana" });
    const after = await gh.getRepo({ org: "UnahurProyects", repoName: "parcial2-ana" });
